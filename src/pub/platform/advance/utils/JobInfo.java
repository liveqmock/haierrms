    ...
    public JobInfo(SYSSCHEDULER scheduler) {
        ...
        if ("1".equals(this.STATUS)) {
            try {
                ...
                System.out.println(this.toString() + "的作业初始化成功，该作业将在【" + startTime + "】第一次启动");

            } catch (ParseException e) {
                System.out.println("编号为【" + JOBID + "】的作业，定时表达式【" + CRONEXPRESSION + "】格式有问题！");
                e.printStackTrace();
    ...
    public void log(Throwable ex) {
        ...
        try {
            ...
            if (ex == null) {
                ...
                update = "update SYS_SCHEDULER set SUCCESSCOUNT=SUCCESSCOUNT+1, LASTEXECUTETIME="
                ...
                update = "update SYS_SCHEDULER set FAILCOUNT=FAILCOUNT+1, LASTEXECUTETIME="
                                + toOracleDateTimeFormat(new GregorianCalendar()) +
                                ", INFORMATION='" + info + "'"
                                + " where JOBID='" + this.JOBID + "'";
            ...
            String insert = "insert into SYS_SCHEDULER_LOG values('"
                            + this.getJOBID()
                            + "','"
                            + this.getJOBNAME()
                            + "',sysdate,'"
