...
import haier.rms.model.IntInfo;
 ...
 * User: zhanrui
 */
public class ExcelIntDataETL extends AbstractExcelETL {
    private static final Log logger = LogFactory.getLog(ExcelIntDataETL.class);
    ...
    public void processCurrYearData(String strStartDate, String strEndDate, String bussItem) {
        ...
        IntInfo key = new IntInfo();
        key.setStartdate(strStartDate);
        key.setDatadate(strEndDate);
        List<IntInfo> list;

        if ("5230".equals(bussItem)) {
            list = session.selectList("InterestInfo.selectAllIntData", key);
        }else if ("5231".equals(bussItem)) {
            list = session.selectList("InterestInfo.selectOutboundIntData", key);
        }else{
            logger.error("国际业务数据处理处理错误。");
            throw new RuntimeException("国际业务数据处理处理错误。");
        }
