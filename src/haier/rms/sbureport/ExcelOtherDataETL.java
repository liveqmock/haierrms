...
import haier.rms.model.OtherIncomeInfo;
 ...
 * User: zhanrui
 */
public class ExcelOtherDataETL extends AbstractExcelETL {
    private static final Log logger = LogFactory.getLog(ExcelOtherDataETL.class);
    ...
    public void processCurrYearData(String strStartDate, String strEndDate, String bussItem) {
        ...
        OtherIncomeInfo key = new OtherIncomeInfo();
        key.setStartdate(strStartDate);
        key.setDatadate(strEndDate);
        List<OtherIncomeInfo> list = session.selectList("InterestInfo.selectOtherData", key);

        int[] days = new int[]{0, 0, 0, 0, 0, 0, 0, 0};
        double[] intrAmt = new double[8];

        try {
            for (OtherIncomeInfo info : list) {
                //计算本年度当前进度实际
    ...
    public double getLastYearTopLevelData(String strCurrDate, String bussItem) {
        ...
        List<OtherIncomeInfo> list;

        list = session.selectList("InterestInfo.selectOtherData", key);
